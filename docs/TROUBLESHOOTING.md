# wgcp — operations & troubleshooting

Practical knowledge for running wgcp on a real Debian 13 host. Pairs with
...
netfilter hooks as our `inet filter` / `inet wgcp` tables and **silently drop**:
...
`inet wgcp`. If you ever see `ip filter` / `ip6 filter`, ufw is back — that is a bug.
...
sudo sqlite3 /var/lib/wgcp/wgcp.db 'select name,wg_ip,enabled,substr(pubkey,1,12) from clients;'
```
No peer → the DB/apply path is the problem (check `journalctl -u wgcp`). Peer present
...
```bash
sudo nft list tables             # want ONLY: inet filter, inet wgcp
```
...
sudo nft list table inet wgcp                # masquerade + spoke policy present?
```
...
cd /opt/wgcp && runuser -u wgcp -- env BUNDLE_GEMFILE=/opt/wgcp/Gemfile HOME=/var/lib/wgcp \
  PATH=/opt/rubies/ruby-4.0.6/bin:/usr/sbin:/usr/bin:/bin \
  ruby -e 'require "console";$LOAD_PATH.unshift("lib");require "wgcp/db";require "wgcp/zone";require "wgcp/reconciler";WGCP::Reconciler.new(WGCP.db,WGCP::Zone.new(WGCP.db)).apply!'
...
| helper socket `EACCES` for the app | `deploy/wgcp-helper.service` | `Group=wgcp` so `RuntimeDirectory` makes `/run/wgcp` root:wgcp |
| app dies on first boot (wg0 not up yet) | `bin/wgcp`, sysctl, `50-bringup.sh` | rescue boot `apply!`; `net.ipv4.ip_nonlocal_bind=1`; restart wgcp after wg-quick |
...
**Boot ordering:** `wgcp.service` is `After=wg-quick@wg0.service` (not `Requires`), so
...
agent — this is exactly what `deploy/run-remote.sh` does when `WGCP_SSH_KEY` is set.
