...
#
#   run-remote.sh <ip> sync              rsync this repo -> /opt/wgcp on the box
...
#
# Env: WGCP_SSH_KEY (optional — a specific private key file; if unset, your normal
# ssh config and agent are used). For `step 50-bringup` or `provision`:
# WGCP_ADMIN_PASSWORD (+ optional WGCP_ENDPOINT_HOST) — shipped to a root-only env
...
if [ -n "${WGCP_SSH_KEY:-}" ]; then
  ...
  SSH_OPTS+=(-o IdentitiesOnly=yes -o IdentityAgent=none -i "$WGCP_SSH_KEY")
fi
...
# when WGCP_ADMIN_PASSWORD is set, then sourcing+removing it. Tees to $2.
run_logged() {
  local remote_cmd="$1" log="$2"
  echo "-> $remote_cmd  (log: $log)" >&2
  if [ -n "${WGCP_ADMIN_PASSWORD:-}" ]; then
    ssh_ "cat >/root/.wgcp-bringup.env && chmod 600 /root/.wgcp-bringup.env" <<EOF
export WGCP_ADMIN_PASSWORD=$(printf '%q' "$WGCP_ADMIN_PASSWORD")
export WGCP_ENDPOINT_HOST=$(printf '%q' "${WGCP_ENDPOINT_HOST:-}")
EOF
    ssh_ "set -a; . /root/.wgcp-bringup.env; rm -f /root/.wgcp-bringup.env; $remote_cmd" 2>&1 | tee "$log"
  else
...
case "$CMD" in
  sync)
    echo "rsync $REPO -> root@$IP:/opt/wgcp" >&2
    ssh_ 'mkdir -p /opt/wgcp'
    rsync -az --delete \
      --exclude '.git' --exclude 'deploy/logs' --exclude '*.db' --exclude '*.db-shm' \
      --exclude '*.db-wal' --exclude '.env' \
      -e "ssh ${SSH_OPTS[*]}" "$REPO"/ "root@$IP:/opt/wgcp/"
    echo "synced" >&2
    ;;
  step)
    step="${1:?which step, e.g. 20-system}"
    run_logged "bash /opt/wgcp/deploy/provision/$step.sh" "$LOGDIR/$(ts)-$step.log"
    ;;
  provision)
    run_logged "bash /opt/wgcp/deploy/provision/provision.sh" "$LOGDIR/$(ts)-provision.log"
    ;;
  exec)
